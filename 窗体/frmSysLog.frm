...
'''            strDateA & "','" & strDateB & "','','" & strMen & "'"
    ...
    strSQL = "SELECT * FROM tb_Test_Sys_OperationLog "
    If Len(strClass) > 0 Then strWhere = strWhere & " AND LogType='" & strClass & "' "
    If Len(strDateA) > 0 And Len(strDateB) > 0 Then strWhere = strWhere & " AND LogTime BETWEEN #" & strDateA & "# AND #" & strDateB & "# "
    If Len(strMen) > 0 Then strWhere = strWhere & " AND LogUserFullName LIKE '%" & strMen & "%' "
    If Len(strInfo) > 0 Then strWhere = strWhere & " AND LogContent LIKE '%" & strInfo & "%' "
...
'''            "CHARINDEX(',',LogUserFullName)) AS [LogUserFullName] FROM tb_Test_Sys_OperationLog"
    ...
    strSQL = "SELECT DISTINCT RIGHT(LogUserFullName,LEN(LogUserFullName)-" & _
            "InStr(LogUserFullName,',')) AS [LogUserFullName] FROM tb_Test_Sys_OperationLog"
            
'    strSQL = "SELECT DISTINCT RIGHT(LogUserFullName,LEN(LogUserFullName)-" & _
'            "InStr(LogUserFullName,',')) AS [LogUserFullName] ,LEN(LogUserFullName) AS [Len] ," & _
'            "InStr(LogUserFullName,',') AS [InStr] FROM tb_Test_Sys_OperationLog"
...
'                Debug.Print rsL.Fields(0), rsL.Fields(1), rsL.Fields(2)
                rsL.MoveNext
            Wend
        End With
    End If
