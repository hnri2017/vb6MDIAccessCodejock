...
'''        .FileSetupNet = .FolderNet & App.EXEName & "Setup.exe"
        ...
        .CnString = gVar.dbConn '连接ACCESS数据库
        .FuncButton = "按钮"
        .FuncControl = "其它"
        .FuncForm = "窗口"
        .FuncMainMenu = "主菜单"
...
'''             "','" & strContent & "','" & gID.UserLoginName & "," & gID.UserFullName & "','" & gID.UserLoginIP & "','" & gID.UserComputerName & "'"
    strSQL = "INSERT INTO tb_Test_Sys_OperationLog " & _
             "(LogType ,LogFormName ,LogTable ,LogContent ,LogUserFullName , " & _
             "LogPCIP ,LogPCName ,LogTime ) " & _
             "VALUES('" & strType & "' ,'" & frmCur.Name & "," & frmCur.Caption & "' ,'" & strTable & "','" & strContent & "','" & gID.UserLoginName & "," & gID.UserFullName & "','" & _
             "" & gID.UserLoginIP & "','" & gID.UserComputerName & "',Now() ) "
